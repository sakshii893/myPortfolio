...
const INERTIA = 0.925;
const PARALLAX_MAG = 0.01; // Reduced from 0.05 to 0.01 for subtle movement
const PARALLAX_EASE = 0.08; // Reduced from 0.12 to 0.08 for smoother movement
...
const ModelInner = ({
  ...
  }, [content, fadeIn, initPitch, initYaw, onLoaded, pivot]);
  ...
  useFrame((_, dt) => {
    ...
    if (Math.abs(vel.current.x) > 1e-4 || Math.abs(vel.current.y) > 1e-4 ||
        ...
        Math.abs(cHov.current.y - tHov.current.y) > 1e-4 || autoRotate) {
      invalidate();
    }
  });
